...
#define LENGTH(object)  sizeof(object) / sizeof(object[0])
...
int leastInterval(char *tasks, int tasksSize, int n)
{
    ...
    int free_slots = (count[last_index] - 1) * n;
    len = count[last_index] + free_slots;

    for (last_index = last_index+1 ; last_index < TOTAL_CHARS ; last_index++) {
        if (count[last_index] != count[0])
            break;

        free_slots -= count[last_index] - 1;
        len++;
    }

    int sum = 0;
    for (int i = last_index ; i < TOTAL_CHARS ; i++) {
        sum += count[i];
    }
